...
export function WizardSummary({ currentStep, data }: { currentStep: number; data: WizardData }) {
  ...
  if (currentStep > 2) {
    ...
    if (data.vehicleType !== "any") {
      parts.push(data.vehicleType.charAt(0).toUpperCase() + data.vehicleType.slice(1));
    }
    ...
    if (parts.length > 0) {
      rows.push({ label: "Vehicle", value: parts.join(" · ") });
    }
  }
  ...
  if (currentStep > 3) {
    ...
    if (data.fuelType !== "any") {
      parts.push(data.fuelType.charAt(0).toUpperCase() + data.fuelType.slice(1));
    }
    if (data.transmission !== "any") {
      parts.push(data.transmission.charAt(0).toUpperCase() + data.transmission.slice(1));
    }
    parts.push(`≤ ${formatMileage(data.maxMileage)}`);
    rows.push({ label: "Specs", value: parts.join(" · ") });
  }
  ...
  if (currentStep > 4) {
    ...
    if (total > 0) {
      const parts = [];
      if (data.requiredFeatures.length > 0) parts.push(`${data.requiredFeatures.length} required`);
      if (data.niceFeatures.length > 0) parts.push(`${data.niceFeatures.length} nice-to-have`);
      rows.push({ label: "Features", value: parts.join(", ") });
    }
  }
  ...
  return (
    <div className="rounded-lg border border-[oklch(0.920_0.006_27.0)] overflow-hidden">
      <div className="px-3 py-2 bg-[oklch(0.977_0.005_27.0)] border-b border-[oklch(0.920_0.006_27.0)]">
        <span className="text-[10px] font-semibold tracking-wide text-[oklch(0.500_0.010_27.0)] uppercase">Your request</span>
      </div>
      <div className="px-3">
        <AnimatePresence>
          {rows.map((row) => (
